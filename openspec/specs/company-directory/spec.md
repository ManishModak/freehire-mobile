...
Browsing and searching the company catalog from the mobile app's Companies
tab: a paginated, debounce-searched list of company cards served by
`GET /api/v1/companies`, presented in the backend's own order, from which a
company opens into the existing company screen (`company-page`).

Scope note: the tab offers no sort control and no facet filters. The web
catalog has both; on mobile they are deliberately absent, so a user who needs
them still goes to the web.
